...
void PrintFile(ClistCtrlEx &m_List_File, CString &dir)
{
	...
	if (nTempName.GetLength()>=5)m_List_File.InsertItem(0, TEXT("此电脑"));
...
BOOL MyFileDlg::OnInitDialog()
{
	...
	m_List_File.InsertColumn(0, TEXT("文件路径"), 0, 650);
	m_List_File.InsertColumn(1, TEXT("创建时间"), 0, 160);
	m_List_File.InsertColumn(2, TEXT("修改时间"), 0, 160);
	m_List_File.InsertColumn(3, TEXT("文件大小"), 0, 130);
