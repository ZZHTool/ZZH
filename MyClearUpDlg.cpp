...
void MyClearUpDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK9, m_update);
}
...
BOOL MyClearUpDlg::OnInitDialog()
{
	...
	m_List_File.InsertColumn(0, TEXT("文件路径"), 0, 450);
...
VOID MyClearUpDlg::CleanSystemTemp()
{
	...
	EmptyDirectory(L"C:\\Windows\\Temp", TRUE);
}

//清理Windows更新补丁
VOID MyClearUpDlg::CleanUpdate()
{
	EmptyDirectory(L"C:\\Windows\\ServiceProfiles\\NetworkService\\AppData\\Local\\Microsoft\\Windows\\DeliveryOptimization\\State", TRUE);
}
...
VOID MyClearUpDlg::CleanFileFindHistory()
{
	...
	CString skey;
	HKEY sub;
	DWORD val = 0x00000001;
	skey = "Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer";
	::RegCreateKey(HKEY_CURRENT_USER, skey, &sub);
	RegSetValueEx(sub, L"ClearRecentDocsonExit", NULL, REG_BINARY, (BYTE*)&val, 4);
	::RegCloseKey(sub);
	EmptyDirectory(L"C:\\ProgramData\\Microsoft\\Search\\Data\\Applications\\Windows", TRUE);
}


//扫描清理线程
static DWORD WINAPI ScanClearUpThread(LPVOID lpThreadParameter)
{
	...
	if (nDlg->m_update.GetCheck())nDlg->CleanUpdate();//清理Windows更新补丁
	...
	CString nTemp;
	if (nDlg->IsScanFile)nTemp.Format(L"扫描完成 垃圾容量：%s", nDlg->GetShowSize(nDlg->m_AllFileSize));
	else nTemp.Format(L"清理完成 共释放容量：%s", nDlg->GetShowSize(nDlg->m_AllFileSize));
...
void MyClearUpDlg::OnBnClickedButton9()
{
	...
	MessageBox(m_szStatus, TEXT("清理完成！"), MB_ICONINFORMATION);
}
...
void MyClearUpDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		CString nTemp;
		nTemp.Format(L"垃圾容量：%s 当前路径：%s", GetShowSize(m_AllFileSize), m_ScaningPath);
		m_Static_Path.SetWindowText(nTemp);
	}
	if (nIDEvent == TIMER_CLEARATH)
	{
		CString nTemp;
		nTemp.Format(L"已清理：%s 当前路径：%s", GetShowSize(m_AllFileSize), m_ScaningPath);
		m_Static_Path.SetWindowText(nTemp);
	}
}
